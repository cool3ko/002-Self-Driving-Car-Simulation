...
- **Fitness**: distance travelled down the road, with a time bonus added if a car
  covers the full `GOAL_DISTANCE` before the generation's frame limit — so evolution
...
Requires Python 3 with `pygame` and `numpy` (see `.venv` / install both via pip).
...
Press **N** during a run to force-skip to the next generation.

## Training log & analysis

Every generation appends one row per car to `logs/training_log.csv`
...
explored in `logs/Data_analysis.ipynb`.
